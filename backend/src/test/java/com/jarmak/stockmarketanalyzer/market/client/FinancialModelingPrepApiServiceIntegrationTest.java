package com.jarmak.stockmarketanalyzer.market.client;

import static org.assertj.core.api.Assertions.assertThat;
import static org.springframework.test.web.client.response.MockRestResponseCreators.withStatus;
import static org.springframework.test.web.client.response.MockRestResponseCreators.withSuccess;

import com.jarmak.stockmarketanalyzer.market.HistoryRange;
import com.jarmak.stockmarketanalyzer.market.MarketModels.ChartPoint;
import java.time.Instant;
import java.util.List;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.params.ParameterizedTest;
import org.junit.jupiter.params.provider.EnumSource;
import org.springframework.http.HttpStatus;
import org.springframework.http.MediaType;
import org.springframework.test.web.client.MockRestServiceServer;
import org.springframework.util.MultiValueMap;
import org.springframework.web.client.RestClient;

class FinancialModelingPrepApiServiceIntegrationTest extends MarketApiClientIntegrationSupport {
  @ParameterizedTest
  @EnumSource(HistoryRange.class)
  void historyRequestsAndReturnsTheSelectedDateRange(HistoryRange range) {
    RestClient.Builder builder = RestClient.builder();
    MockRestServiceServer server = MockRestServiceServer.bindTo(builder).build();
    FinancialModelingPrepApiService service =
        new FinancialModelingPrepApiService(properties(null, null, null, "fmp", null), builder.build());

    Instant start = fixtureStart(range);
    Instant end = fixtureEnd();
    server.expect(request -> {
          assertThat(request.getURI().getHost()).isEqualTo("financialmodelingprep.com");
          assertThat(request.getURI().getPath()).isEqualTo(financialModelingPrepPath(range));
          MultiValueMap<String, String> params = queryParams(request.getURI().toString());
          assertThat(params.getFirst("symbol")).isEqualTo("AAPL");
          assertThat(params.getFirst("apikey")).isEqualTo("fmp");
          assertFromToDateRequestRange(params, range);
        })
        .andRespond(withSuccess(financialModelingPrepHistoryPayload(start, end), MediaType.APPLICATION_JSON));

    List<ChartPoint> points = service.historicalCandles("AAPL", range);

    assertReturnedRange(points, range);
    server.verify();
  }

  @Test
  void rateLimitDoesNotCacheEmptyHistory() {
    RestClient.Builder builder = RestClient.builder();
    MockRestServiceServer server = MockRestServiceServer.bindTo(builder).build();
    FinancialModelingPrepApiService service =
        new FinancialModelingPrepApiService(properties(null, null, null, "fmp", null), builder.build());
    HistoryRange range = HistoryRange.ONE_YEAR;
    Instant start = fixtureStart(range);
    Instant end = fixtureEnd();

    server.expect(request -> assertThat(request.getURI().getPath()).isEqualTo("/stable/historical-price-eod/full"))
        .andRespond(withStatus(HttpStatus.TOO_MANY_REQUESTS).body("""
            {"Error Message":"Rate limited"}
            """));
    server.expect(request -> assertThat(request.getURI().getPath()).isEqualTo("/stable/historical-price-eod/full"))
        .andRespond(withSuccess(financialModelingPrepHistoryPayload(start, end), MediaType.APPLICATION_JSON));

    assertThat(service.historicalCandles("AAPL", range)).isEmpty();

    List<ChartPoint> points = service.historicalCandles("AAPL", range);

    assertReturnedRange(points, range);
    server.verify();
  }

  private String financialModelingPrepPath(HistoryRange range) {
    return switch (range) {
      case ONE_HOUR -> "/stable/historical-chart/5min";
      case ONE_DAY -> "/stable/historical-chart/1hour";
      default -> "/stable/historical-price-eod/full";
    };
  }
}
